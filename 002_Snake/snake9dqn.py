...
class SnakeGame:
    ...
    def reset(self):
        # Start snake in the center, stored as a tuple (for hashability)
        center_x = (self.width // (2 * self.grid_size)) * self.grid_size
        center_y = (self.height // (2 * self.grid_size)) * self.grid_size
        self.snake = (center_x, center_y)
        # Maintain both an ordered list (for tail removal) and a set (for fast collision checks)
        self.snake_body = []       # List of tuples (ordered)
        self.snake_body_set = set()  # Set of tuples for O(1) membership tests
    ...
    def place_food(self):
        while True:
            ...
            food_candidate = (food_x, food_y)
            # Ensure food isn't placed on the snake
            if food_candidate not in self.snake_body_set and food_candidate != self.snake:
    ...
    def get_state(self):
        ...
        # Check dangers using the set for snake_body collision detection
        ...
        def is_danger(direction):
            ...
            return (
                ...
                next_pos in self.snake_body_set
            )
    ...
    def step(self, action):
        ...
        # Store the old head position (will become a body segment)
        old_head = self.snake
        ...
        if (x < 0 or x >= self.width or
            ...
            new_head in self.snake_body_set):
        ...
        if new_head == self.food:
            self.score += 1
            reward = APPLE_REWARD + len(self.snake_body) * 0.5
            # When food is eaten, we do NOT remove the tail (snake grows).
            self.place_food()
        else:
            reward = -0.1  # Movement penalty.
            # Remove the tail if the snake has already grown (score > 0).
            if self.score > 0 and self.snake_body:
                tail = self.snake_body.pop(0)
                self.snake_body_set.remove(tail)

        # Add the previous head as the new body segment if the snake has grown.
        if self.score > 0:
            self.snake_body.append(old_head)
            self.snake_body_set.add(old_head)

        # Optional: Penalize loops and reward exploration.
        if self.snake_body[-5:] and new_head in self.snake_body[-5:]:
            reward -= 2
        if self.snake_body[:-5] and new_head not in self.snake_body[:-5]:
            reward += 0.1
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        self.state_size = state_size  # e.g., 9 for our snake state
        self.action_size = action_size  # e.g., 3 possible actions
        self.gamma = GAMMA  # Discount factor
        self.learning_rate = LEARNING_RATE
        self.epsilon = START_EPSILON  # Starting exploration rate
        self.epsilon_min = END_EPSILON
        self.epsilon_decay = EPSILON_DECAY  # Decay factor per training step/episode
        self.replay_buffer = deque(maxlen=2000)
        self.batch_size = 64
        ...
        # How often to update the target model (in episodes or training iterations)
    ...
    def get_action(self, state):
        ...
        :param state: numpy array of shape (state_size,)
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        batch = random.sample(self.replay_buffer, self.batch_size)
        # Prepare separate arrays for each component.
        ...
        # Train the primary network on the updated Q-values.
    ...
    def maybe_update_target(self, episode):
        """
        Update the target model periodically.
        """
        # For example, update the target network every N episodes.
...
def train_agent():
    ...
    # Switch to DQNAgent for deep Q-learning
